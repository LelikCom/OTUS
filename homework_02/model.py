...
class PhoneBook:
    ...
    def edit_contact(self, contact_id: int) -> Optional[dict]:
        ...
        contact_to_edit = next((contact for contact in self.contacts if contact['id'] == contact_id), None)
        if contact_to_edit:
            print(text_ru.msg_contact_edit(contact_to_edit))
            # print(f"Контакт найден: ID {contact_to_edit['id']}")
    ...
    def delete_contact(self, contact_id: int) -> Optional[dict]:
        ...
        contact_to_delete = next((contact for contact in self.contacts if contact['id'] == contact_id), None)
...
class Contact:
    ...
    def get_next_id(self) -> int:
        ...
        if not self.phone_book.contacts:
            return 1
        existing_ids = sorted(contact['id'] for contact in self.phone_book.contacts)
