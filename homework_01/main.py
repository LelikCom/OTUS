...
def edit_contact(contacts: List[Dict[str, Any]]) -> None:
    ...
    search_query = input(
        "Введите имя, телефон или комментарий для поиска контакта для редактирования: "
    )
    results = search_contacts(contacts, search_query)
    ...
    if results:
        ...
        contact_to_edit = next(
            (contact for contact in contacts if contact["id"] == contact_id), None
        )

        if contact_to_edit:
            contact_to_edit["name"] = (
                input(f"Новое имя (текущие: {contact_to_edit['name']}): ")
                or contact_to_edit["name"]
            )
            contact_to_edit["phone"] = (
                input(f"Новый телефон (текущие: {contact_to_edit['phone']}): ")
                or contact_to_edit["phone"]
            )
            contact_to_edit["comment"] = (
                input(f"Новый комментарий (текущие: {contact_to_edit['comment']}): ")
...
def delete_contact(contacts: List[Dict[str, Any]]) -> None:
    ...
    search_query = input(
        "Введите имя, телефон или комментарий для поиска контакта для удаления: "
    )
    results = search_contacts(contacts, search_query)
    ...
    if results:
        ...
        if contact_to_delete:
            confirm = input(
                f"Вы точно хотите удалить контакт с ID {contact_id} (да/нет)? "
            ).lower()
            if confirm == "да":
...
def main_menu() -> None:
    ...
    while is_running:
        ...
        if choice == "1":
            ...
            if (
                input("Вы хотите сохранить изменения перед выходом? (да/нет): ").lower()
                == "да"
            ):
                save_contacts(contacts)
